...
class AddBookingDetailsVC: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func btnModifyClick(_ sender: UIButton) {
        ...
        if error == ""{
            self.addOrder(email: GFunction.user.emailId!, hospitalName: self.data.hospitalName, firstName: (self.txtName.text?.trim())!, lastName: (self.txtLastName.text?.trim())!, age: (self.txtAge.text?.trim())!, gender: (self.txtGender.text?.trim())!, date: (self.txtDate.text?.trim())!)
        }else{
    ...
    func addOrder(email:String,hospitalName: String,firstName:String,lastName:String,age:String,gender:String,date:String) {
        var ref : DocumentReference? = nil
       
        ref = AppDelegate.shared.db.collection(kOrder).addDocument(data:
            [
              kEmail: email,
              kHospitalName: hospitalName,
        ...
        {  err in
            if let err = err {
                ...
                if let vc = UIStoryboard.main.instantiateViewController(withClass: BookingSuccessVC.self) {
...
extension AddBookingDetailsVC  :UIPickerViewDelegate,UIPickerViewDataSource {
